...
ssh -o StrictHostKeyChecking=no -p 22 ConMan@${ps1}.emulab.net "python3 index.py&"

# Run Namescheduler.py in the next node
ssh -o StrictHostKeyChecking=no -p 22 ConMan@${ps2}.emulab.net "python3 NameScheduler.py&"


# Run Node.py in the rest of the nodes
for pcname in ${pc} ; do
    ssh -o StrictHostKeyChecking=no -p 22 ConMan@${pcname}.emulab.net "python3 Node.py"
done
